...
Page({
  ...
  data: {
    ...
    statusTypeItems: [
      { value: 1, name: '用于报名和面试通知', checked: 'true'},
      { value: 2, name: '用于面试签到'}
    ],
  ...
  bindStatusChange: function (e) {
    var newStatus = e.detail.value;
    var tmpArray = this.data.statusTypeItems
    for (var i = 0, len = tmpArray.length; i < len; ++i) {
      tmpArray[i].checked = (tmpArray[i].value == newStatus)
    }

    this.setData({
      statusTypeItems: tmpArray,
      registTableStatus: newStatus
    })
  },
  ...
  createRegistTable: function () {
    ...
    wx.request({
      ...
      data: {
        ...
        registTableStatus: this.data.registTableStatus,
